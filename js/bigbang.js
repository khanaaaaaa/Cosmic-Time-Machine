function createBigBangAnimation() {
    ...
    for (let i = 0; i < count; i++) {
        ...
        c.setHSL(Math.random() * 0.12, 1.0, 0.5 + Math.random() * 0.5);
        col.push(c.r, c.g, c.b);
        const theta = Math.random() * Math.PI * 2;
        const phi   = Math.acos(2 * Math.random() - 1);
        const speed = 20 + Math.random() * 250;
        vel.push(
...
function triggerBigBang() {
    ...
    setTimeout(() => { scene.background = null; }, 200);
}
...
function updateBigBang(delta) {
    ...
    const drag = Math.max(0.2, 1 - explosionTime * 0.15);
    ...
    bigBangParticles.geometry.attributes.position.needsUpdate = true;
    bigBangParticles.material.opacity = Math.max(0, 1 - explosionTime / 5);
    bigBangParticles.material.size    = 4 + explosionTime * 1.5;
    if (explosionTime > 5) {
        bigBangActive = false;
        bigBangParticles.visible = false;
    }
