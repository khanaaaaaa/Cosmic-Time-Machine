function createGalaxy(color) {
    ...
    for (let j = 0; j < 3000; j++) {
        ...
        const angle  = (arm / arms) * Math.PI * 2 + spin + (Math.random() - 0.5) * spread;
        const height = (Math.random() - 0.5) * (1 - t) * 18;
    ...
    for (let j = 0; j < 500; j++) {
        ...
        pos.push(Math.cos(a) * r, (Math.random() - 0.5) * 4, Math.sin(a) * r);
        col.push(1.0, 0.95, 0.8);
    }
...
function updateCollision() {
    ...
    if (dist < 80) {
        collidingGalaxies.forEach(gal => {
            ...
            if (gal.material) gal.material.opacity = 0.6 + Math.random() * 0.4;
        });
    }
...
function updateGalaxies() {
    ...
    while (galaxies.length < needed) {
        const gal = createGalaxy(new THREE.Color().setHSL(Math.random(), 0.8, 0.5));
        gal.position.set(
    ...
    while (galaxies.length > needed) scene.remove(galaxies.pop());
}
